...
from weight_calculator import get_scores
...
def main():
    ...
    print(get_scores(p, m, s, l, '5 star', main_sub_weight, dps_weights_main, dps_weights_sub, main_weight_tiers, sub_weight_tiers))
...
def get_sub_stats(sub_stats):
    ...
    for sub in sub_stats:
        if '%' in sub:
            parsed_subs[sub[:sub.index('+')] + '%'] = float(sub[sub.index('+')+1:-1])
        else:
            parsed_subs[sub[:sub.index('+')]] = float(sub[sub.index('+')+1:])
