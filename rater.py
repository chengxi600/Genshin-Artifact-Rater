...
def main():
    ...
    p, m, s = process_parsed_text(result)
    print(calculate_stat_weights(p, m, s, dps_weights_main, dps_weights_sub, main_weight_tiers, sub_weight_tiers))
...
def process_parsed_text(parsedInfo):
    ...
    return piece, main_stat, get_sub_stats(sub_stats)

#creates a dictionary of stat name: stat number
def get_sub_stats(sub_stats):
    parsed_subs = {}
    for sub in sub_stats:
        if '%' in sub:
            parsed_subs[sub[:sub.index('+')] + '%'] = sub[sub.index('+')+1:-1]
        else:
            parsed_subs[sub[:sub.index('+')]] = sub[sub.index('+')+1:]
